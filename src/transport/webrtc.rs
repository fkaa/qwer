use webrtc::{
    ...
    media::{
        rtp::rtp_codec::RTCRtpCodecCapability,
...
use crate::{
    ...
    AppData, ContextLogger, FilterGraph, Frame, FrameReadFilter, FrameWriteFilter,
};
...
impl FrameWriteFilter for WebRtcWriteFilter {
    ...
    async fn write(&mut self, frame: Frame) -> anyhow::Result<()> {
        if let Some(WebRtcTrack { track, .. }) =
            self.tracks.iter().find(|t| t.stream.id == frame.stream.id)
        {
...
async fn handle_websocket_video_response(
    ...
    if let Some(mut frame_queue) = frame_queue {
        ...
        let tracks = get_tracks_from_streams(
            streams
                .into_iter()
                .filter(|s| s.is_video())
                .collect::<Vec<_>>(),
        ...
        for track in &tracks {
            debug!(
                logger,
                "Adding WebRTC track for mime type '{}'",
                track.track.codec().mime_type
            );
            let rtp_sender = peer_connection
                .add_track(Arc::clone(&track.track) as _)
                .await?;
        ...
        socket
            .send(Message::Text(
                serde_json::to_string(&SignalingMessage::Offer(offer)).unwrap(),
            ))
            .await
            .unwrap();
        ...
        tokio::spawn(async move {
            ...
            let write_filter = Box::new(BitstreamFramerFilter::new(
                blogger.clone(),
